...
def main() -> None:
    ...
    if "passes" in selected_sources:
        dataset_raw = carry_forward_cumulative_features(dataset_raw, PASS_CUMUL_COLS)
        for col in PASS_FEATURE_COLS:
            if col in dataset_raw.columns:
                dataset_raw[col] = dataset_raw[col].fillna(0).astype(int)
    if "pressure" in selected_sources:
        dataset_raw = carry_forward_cumulative_features(
            dataset_raw,
            PRESSURE_CUMUL_COUNT_COLS + PRESSURE_CUMUL_RATE_COLS,
        )
    ...
    if "runs" in selected_sources:
        dataset_raw = carry_forward_cumulative_features(dataset_raw, RUN_CUMUL_COLS)
    ...
    if "shots" in selected_sources:
        dataset_raw = carry_forward_cumulative_features(
            dataset_raw,
            SHOT_CUMUL_COUNT_COLS + SHOT_CUMUL_RATE_COLS,
        )
