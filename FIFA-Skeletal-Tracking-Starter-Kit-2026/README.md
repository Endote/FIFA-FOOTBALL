...
- **Support-Point Anchoring**: Places each player on the pitch using foot-driven support points with bbox fallback.
...
- Player placement is driven by an estimated support point on the pitch (ankle midpoint, then foot joints, then bbox bottom-center), which is more stable than using a torso or mask centroid.
