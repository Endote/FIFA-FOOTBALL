...
RIGHT_TOE_IDX = 13
LEFT_TOE_IDX = 14
ANKLE_IDXS = (RIGHT_ANKLE_IDX, LEFT_ANKLE_IDX)
FOOT_IDXS = (RIGHT_ANKLE_IDX, LEFT_ANKLE_IDX, RIGHT_TOE_IDX, LEFT_TOE_IDX)
LOWER_BODY_IDXS = (7, 8, 9, 10, RIGHT_ANKLE_IDX, LEFT_ANKLE_IDX, RIGHT_TOE_IDX, LEFT_TOE_IDX)
...
def estimate_support_point_2d(skel_2d, box):
    """Estimate the image-space ground contact point from feet first, box second."""
    ankle_points = skel_2d[list(ANKLE_IDXS)]
    if np.isfinite(ankle_points).all():
        return ankle_points.mean(axis=0), "ankle_midpoint"

    valid_foot_idxs = [idx for idx in FOOT_IDXS if np.isfinite(skel_2d[idx]).all()]
    if valid_foot_idxs:
        support_idx = max(valid_foot_idxs, key=lambda idx: skel_2d[idx, 1])
        return skel_2d[support_idx], support_idx

    if np.isfinite(box).all():
        return np.array([(box[0] + box[2]) * 0.5, box[3]], dtype=np.float32), "bbox_bottom"

    return None, None


def estimate_support_anchor_3d(skel_3d, anchor_spec):
    """Select the 3D point that should be snapped to the pitch support point."""
    if anchor_spec == "ankle_midpoint":
        ankle_points = skel_3d[list(ANKLE_IDXS)]
        if np.isfinite(ankle_points).all():
            return ankle_points.mean(axis=0)

    if isinstance(anchor_spec, int) and np.isfinite(skel_3d[anchor_spec]).all():
        return skel_3d[anchor_spec]

    foot_points = skel_3d[list(FOOT_IDXS)]
    valid_foot_mask = np.isfinite(foot_points).all(axis=-1)
    if valid_foot_mask.any():
        return foot_points[valid_foot_mask].mean(axis=0)

    lower_body_points = skel_3d[list(LOWER_BODY_IDXS)]
    valid_lower_body_mask = np.isfinite(lower_body_points).all(axis=-1)
    if valid_lower_body_mask.any():
        return lower_body_points[valid_lower_body_mask].mean(axis=0)

    valid_joint_mask = np.isfinite(skel_3d).all(axis=-1)
    if valid_joint_mask.any():
        return skel_3d[valid_joint_mask].mean(axis=0)

    return None


def extract_support_tracks(predictions, skels_2d, boxes):
    """Collect per-track support anchors for refinement."""
    num_persons, num_frames = predictions.shape[:2]
    support_3d = np.full((num_persons, num_frames, 3), np.nan, dtype=np.float32)
    support_2d = np.full((num_persons, num_frames, 2), np.nan, dtype=np.float32)
    ...
    for person in range(num_persons):
        for frame_idx in range(num_frames):
            ...
            support_point_2d, anchor_spec = estimate_support_point_2d(skels_2d[frame_idx, person], box)
            if support_point_2d is None:
                continue

            support_anchor_3d = estimate_support_anchor_3d(predictions[person, frame_idx], anchor_spec)
            if support_anchor_3d is None:
                continue

            support_2d[person, frame_idx] = support_point_2d
            support_3d[person, frame_idx] = support_anchor_3d

    return support_3d, support_2d
...
def fine_tune_translation(predictions, skels_2d, cameras, Rt, boxes):
    """Fine-tune translation using support points rather than torso-centric anchors."""
    NUM_PERSONS = predictions.shape[0]
    support_3d, support_2d = extract_support_tracks(predictions, skels_2d, boxes)
    ...
    valid = (~np.isnan(boxes).any(axis=-1).transpose(1, 0)
             & ~np.isnan(support_3d).any(axis=-1)
             & ~np.isnan(support_2d).any(axis=-1))
    if not valid.any():
        return torch.empty((0, 3), dtype=torch.float32, device="cuda"), valid

    traj_3d = minimize_reprojection_error(
        pts_3d=torch.tensor(support_3d[valid], dtype=torch.float32).to("cuda"),
        pts_2d=torch.tensor(support_2d[valid], dtype=torch.float32).to("cuda"),
...
def process_sequence(
    ...
    for frame_idx in (pbar := tqdm(range(NUM_FRAMES), desc=f"{video_path.stem}")):
        ...
        for person in range(NUM_PERSONS):
            ...
            support_point_2d, anchor_spec = estimate_support_point_2d(skel_2d, box)
            if support_point_2d is None:
                continue
            x, y = support_point_2d
            K = cameras["K"][frame_idx]
            k = cameras["k"][frame_idx]
            R, t = Rt[-1]
            ...
            skel_3d = skel_3d @ R
            support_anchor_3d = estimate_support_anchor_3d(skel_3d, anchor_spec)
            if support_anchor_3d is None:
                continue
            skel_3d = skel_3d - support_anchor_3d + intersection
